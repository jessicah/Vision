...
ChannelAgent::SortNames(const void *name1, const void *name2)
{
	...
	if ((firstPtr)->Name()[0] == '~')
		first << STATUS_FOUNDER_BIT;
	else if ((firstPtr)->Name()[0] == '&')
		first << STATUS_PROTECTED_BIT;
	else if ((firstPtr)->Status() <= STATUS_OP_BIT)
		first << STATUS_OP_BIT;
	else if ((firstPtr)->Status() <= STATUS_HELPER_BIT)
		first << STATUS_HELPER_BIT;
	else if ((firstPtr)->Status() <= STATUS_VOICE_BIT)
		first << STATUS_VOICE_BIT;
	else
		first << (firstPtr)->Status();
	
	if ((secondPtr)->Name()[0] == '~')
		second << STATUS_FOUNDER_BIT;
	else if ((secondPtr)->Name()[0] == '&')
		second << STATUS_PROTECTED_BIT;
	else if ((secondPtr)->Status() <= STATUS_OP_BIT)
		second << STATUS_OP_BIT;
	else if ((secondPtr)->Status() <= STATUS_HELPER_BIT)
		second << STATUS_HELPER_BIT;
	else if ((secondPtr)->Status() <= STATUS_VOICE_BIT)
		second << STATUS_VOICE_BIT;
	else
