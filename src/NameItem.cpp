...
NameItem::DrawItem (BView *father, BRect frame, bool complete)
{
	...
	if (drawString[0] == '~')
		color = activeTheme->ForegroundAt (C_MIRC_GREEN);
	else if (drawString[0] == '&')
		color = activeTheme->ForegroundAt (C_MIRC_BLUE);
	...
	{
		drawString.Prepend ("%");
		color = activeTheme->ForegroundAt (C_MIRC_TEAL);
	}
	else if ((myStatus & STATUS_VOICE_BIT) != 0)
	{
		drawString.Prepend("+");
		color = activeTheme->ForegroundAt (C_MIRC_ORANGE);
	}
